...
public partial class MainWindow : Window
{
    ...
    /// </summary>
    private readonly HashSet<string> _writableePaths = new(StringComparer.OrdinalIgnoreCase);
    ...
    public void HandleExternalOpen(IEnumerable<string> paths)
    {
        ...
        foreach (var p in list) _writableePaths.Add(Path.GetFullPath(p));
    ...
    private async Task<object?> DispatchAsync(string op, JsonElement args)
    {
        switch (op)
        {
            ...
            case "fileAssociation":
                return SetFileAssociation(Bool(args, "enable", true));
    ...
    private object? OpenFileDialog(bool multi)
    {
        ...
        foreach (var p in dlg.FileNames) _writableePaths.Add(Path.GetFullPath(p));
    ...
    private object WriteTextFile(string path, string content, string encodingName, string eol)
    {
        ...
        if (!_writableePaths.Contains(full))
    ...
    // ------------------------------------------------ associacao de arquivo

    private const string ProgId = "MarkPad.Document.1";

    private static bool IsFileAssociated()
    {
        try
        {
            ...
            return cmd is not null && exe is not null &&
                   cmd.Contains(exe, StringComparison.OrdinalIgnoreCase);
        }
        catch { return false; }
    }

    /// <summary>
    /// Registra (ou remove) o MarkPad como aplicativo para .md, somente em HKCU.
    /// Disparado pelo usuario no menu; reversivel pelo mesmo caminho.
    /// </summary>
    private static object SetFileAssociation(bool enable)
    {
        var exe = Environment.ProcessPath
        ...
        if (!enable)
        {
            foreach (var ext in extensions)
            {
                ...
                if (extKey?.GetValue(null) as string == ProgId) extKey.DeleteValue("", throwOnMissingValue: false);
        ...
        {
            progKey.SetValue("", "Documento Markdown");
        ...
        foreach (var ext in extensions)
        {
            ...
            using var progids = extKey.CreateSubKey("OpenWithProgids");
            progids.SetValue(ProgId, Array.Empty<byte>(), RegistryValueKind.None);
        }

        return new { associated = true };
    }
}
